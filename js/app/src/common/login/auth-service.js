...
.factory('AuthProvider', ['$http', 'SessionService', 'Auth', 'config', '$q', 
    function ($http, SessionService, Auth, config, $q) {
        return {
            login: function (credentials) {  
                if (config.liveData) {
                    return Auth.login(credentials);
                } else {
                    return $http.get('/js/app/data/user.json');
                }
            },
            logout: function () {  
                if (config.liveData) {
                    return Auth.logout(SessionService.get('userId'));
                } else {
                    return $http.get('/js/app/data/logout.json');
                }
            },
...
.factory('Auth', ['$resource', function($resource) {
    return $resource('/login/:userId/:password/:token', 
    { userId: 'userId', password: 'password', token: 'token' }, { 
        login: { 
            method: 'PUT', 
            params: { userId: '@userId', password: '@password', token: '@token' }, 
            isArray: false 
        },
        logout: {
            method: 'GET', 
